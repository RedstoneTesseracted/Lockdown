# Determine how many items (Drone Control Tower) can be crafted
execute store result score $ld_result ld_result run data get block ~ ~ ~ Items[{Slot:2b}].Count
...
item replace block ~ ~ ~ container.16 with gray_glazed_terracotta{LockdownItem:1b,LockdownData:{id:19,function_model:10016,block_model:10016,item_model:10016,block_id:3,item_id:3,channel:0},CustomModelData:10016,display:{Name:'{"text":"Drone Control Tower","italic":"false"}',Lore:['{"text":"No Code Assigned","color":"red"}']}}
