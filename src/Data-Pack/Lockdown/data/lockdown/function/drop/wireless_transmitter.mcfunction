# This drops and configures a wireless transmitter item at the current position.
#
...
item modify entity @n[tag=lockdown.item.configure] container.0 lockdown:item/wireless_transmitter

# Set channel data
execute store result entity @n[tag=lockdown.item.configure] Item.components."minecraft:entity_data".data.lockdown_data.channel int 1.0 run scoreboard players get lockdown.channel lockdown.local
