...
execute if score lockdown.channel lockdown.local matches 1.. run execute store result entity @n[tag=lockdown.item.configure] Item.components."minecraft:entity_data".data.lockdown_data.channel int 1.0 run scoreboard players get lockdown.channel lockdown.local
