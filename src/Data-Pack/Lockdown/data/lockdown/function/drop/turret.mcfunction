# This drops and configures a turret item at the current position.
#
...
execute if score lockdown.channel lockdown.local matches 1.. run execute store result entity @n[tag=lockdown.item.configure] Item.components."minecraft:entity_data".Item.components."minecraft:custom_data".lockdown_data.channel int 1.0 run scoreboard players get lockdown.channel lockdown.local
