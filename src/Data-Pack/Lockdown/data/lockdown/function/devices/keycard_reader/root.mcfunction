...
execute at @a[distance=..4,nbt={SelectedItem:{tag:{LockdownData:{id:4}}}}] if score @s ld_channel = @p[limit=1,sort=nearest,distance=..0.1] ld_channel_id run tag @s add ld_found_card
...
execute unless block ~ ~ ~ minecraft:acacia_button run function lockdown:devices/keycard_reader/destroy