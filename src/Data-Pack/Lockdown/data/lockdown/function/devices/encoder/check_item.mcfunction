...
execute store result score @s ld_result if block ~ ~ ~ minecraft:dispenser{Items:[{Slot:4b,tag:{LockdownData:{}}}]} unless block ~ ~ ~ minecraft:dispenser{Items:[{Slot:4b,tag:{LockdownData:{channel:0}}}]} run function lockdown:devices/encoder/check_dye
