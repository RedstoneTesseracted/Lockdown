...
execute if predicate lockdown:vehicle_target_invulnerable run return 0
data modify storage lockdown:temp args set value {r: 0}
execute store result storage lockdown:temp args.r int 1 run scoreboard players get @s lockdown.pursuit_exit_range
